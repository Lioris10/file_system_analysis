...
The first coded MVP implements the backend scan/summarize pipeline and a CLI entry point. The UI remains specified but not yet implemented. The backend includes YAML settings, file metadata scanning, document reader selection, TXT/MD/CSV extraction, optional PDF/Office readers, a deterministic fallback summarizer, and a lazy LangChain model factory.

The deterministic summarizer allows local development and tests to run without network access or API keys. Live LLM summarization should be added as a LangGraph node that uses the existing `ModelFactory` integration point.
